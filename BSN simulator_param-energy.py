...
class CustomMainWindow(QMainWindow): # about the main window
    ...
    def add_matrix(self) : # add matrix
        self.label_J = QLabel(self)
        self.label_J.setText('J')
        self.label_J.setFont(QFont("맑은 고딕", 18)) #폰트,크기 조절
        self.label_J.setStyleSheet('QLabel {padding: 1px;}')
        self.LAYOUT_C.addWidget(self.label_J, self.inc_flag + 1, 0)
            
        for i in range(self.graph_num) :
            for j in range(self.graph_num) :
                globals()['textbox_{}'.format('J_' + str(i) + str(j))] = QLineEdit(self)
                globals()['textbox_{}'.format('J_' + str(i) + str(j))].resize(10,10)
                globals()['textbox_{}'.format('J_' + str(i) + str(j))].setFont(QFont("맑은 고딕", 15)) #폰트,크기 조절
                globals()['textbox_{}'.format('J_' + str(i) + str(j))].setText('0')
                self.LAYOUT_C.addWidget(globals()['textbox_{}'.format('J_' + str(i) + str(j))], self.inc_flag + 1, j + 1)
            self.inc_flag += 1
            
        self.label_H = QLabel(self)
        self.label_H.setText('H')
        self.label_H.setFont(QFont("맑은 고딕", 18)) #폰트,크기 조절
        self.label_H.setStyleSheet('QLabel {padding: 1px;}')
        self.LAYOUT_C.addWidget(self.label_H, self.inc_flag + 1, 0)
            
        for i in range(self.graph_num) :
            globals()['textbox_{}'.format('H_' + str(i))] = QLineEdit(self)
        ...
        self.label_I0.setFont(QFont("맑은 고딕", 18)) #폰트,크기 조절
        self.label_I0.setStyleSheet('QLabel {padding: 1px;}')
        self.label_I0.setAlignment(Qt.AlignRight)
        self.LAYOUT_C.addWidget(self.label_I0, self.inc_flag + 1, 2)
    ...
    def save_click(self, state) : # save parameter value
        ...
        for i in range(self.graph_num):
            for j in range(self.graph_num) :
                if globals()['textbox_{}'.format('J_' + str(i) + str(j))].text() != "" :
                    pa_list.append(globals()['textbox_{}'.format('J_' + str(i) + str(j))].text())
    ...
    def load_click(self, state) : # load parameter value
        ...
        if gr_n == self.graph_num :
            for i in range(gr_n):
                globals()['textbox{}'.format(i + 1)].setText(pa_list[flag + 1])
                flag += 1
                
            for i in range(gr_n):
                globals()['textbox_relt{}'.format(i + 1)].setText(pa_list[flag + 1])  
                flag += 1
                
            textbox_samt.setText(pa_list[flag + 1])
            flag += 1
            textbox_samn.setText(pa_list[flag + 1])
            flag += 1
            
            for i in range(gr_n):
                for j in range(gr_n) :
                    globals()['textbox_{}'.format('J_' + str(i) + str(j))].setText(pa_list[flag + 1])
                    flag += 1
