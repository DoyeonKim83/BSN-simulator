...
class CustomMainWindow(QMainWindow): # about the main window
    def __init__(self):
        ...
        self.setWindowTitle("BSN Graph_energy")
    ...
    def load_click(self) :  
        ...
        for i in range(self.graph_num):
            globals()['box_input{}'.format(chr(i + 1 + 64))] = float(param_list[flag + 1])
            pa_list.append(globals()['box_input{}'.format(chr(i + 1 + 64))])
            flag += 1
            
        for i in range(self.graph_num):
            globals()['relt_input{}'.format(chr(i + 1))] = float(param_list[flag + 1])
            re_list.append(globals()['relt_input{}'.format(chr(i + 1))])
            flag += 1
                
        self.samt = float(param_list[flag + 1])
        flag += 1
        self.samn = int(param_list[flag + 1])
        flag += 1
        
        for i in range(self.graph_num):
            for j in range(self.graph_num) :
                globals()['matrix_{}'.format('J_' + str(i) + str(j))] = int(param_list[flag + 1])
                matrix_list.append(globals()['matrix_{}'.format('J_' + str(i) + str(j))])
                flag += 1
                    
        for i in range(self.graph_num) :
            globals()['matrix_{}'.format('H_' + str(i))] = int(param_list[flag + 1])
    ...
    def reload_click(self) : # reload and set parameter value
        ...
        self.samt = float(param_list[flag + 1])
        flag += 1
        self.samn = float(param_list[flag + 1])
        flag += 1
...
class ConfigurationCanvas(FigureCanvas):
    ...
    def animate(self, i):
        if self.ani_flag == 1 and len(self.plus) == self.samn : # FuncAnimation이 start 상태이고 sampling 개수가 채워진 상태이면
            ...
            self.ax.set_xlabel('State (decimal)')
            self.ax.set_ylabel('Probability')
            self.hi, bins, self.patches = self.ax.hist(self.plus, bins=range(0, self.rg, 1), rwidth=0.8, color='#abffb6', density=True)
            self.init_plus()
        return self.patches
...
class CustomFigCanvas1(FigureCanvas, TimedAnimation): # for constant graph
        ...
        for i in range(self.graph_num) :
            for j in range(num) :
                ...
                if state == 0 : # init
                    ...
                    globals()['{}_1'.format(chr(i + 97))].append(globals()['s{}_1'.format(chr(i + 65))])
    ...
    def change_matrix(self, I0, *matrix): # matrix value 및 관련 parameter value update
        ...
        for i in range(self.graph_num) :
            for j in range(self.graph_num) :
                self.j[i][j] = matrix[flag]
                flag += 1
        print(self.j)
        
        for i in range(self.graph_num) :
            self.h[i] = matrix[flag]
            flag += 1
        print(self.h)
        
        timer.setInterval(self.samt * 1000) # timer interval init
        self.confFig.change_samn(self.samn)
        for i in range(self.graph_num) : # queue init
            globals()['queue{}'.format(chr(i + 65))] = Queue(self.samn)
        self.samn_flag = 0
